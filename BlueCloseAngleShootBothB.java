...
@Autonomous(name="Compbot: BlueCloseAngleShootBothB", group="BLUE")
@Disabled
public class BlueCloseAngleShootBothB extends LinearOpMode {
    ...
    /*  Settings for Competition robot
    ...
    /*Settings for TEST ROBOT LITTLE GUY*/
    // These constants define the desired driving/control characteristics
    // The can/should be tweaked to suite the specific robot drive train.
    static final double     DRIVE_SPEED             = 0.5;     // Nominal speed for better accuracy.
    ...
    public void runOpMode() {
        ...
        //gyroDrive(DRIVE_SPEED,   10.0, 0.0);    // Drive forward at 0 degrees
        //gyroTurn( TURN_SPEED,   -20.0);         // Turns towards beacon
        //gyroHold( TURN_SPEED,   -20.0, 0.5);    // Holds turn
        gyroDrive(0.6,  40.0, 0.0);    // Drive forward at -20 degrees (94 inches to reach white line without ODS)
        //encoderDrive(0.2, 10.0, -10.0, 3.0);    //Used to turn robot to beacon to begin following line
        gyroTurn( TURN_SPEED,   -70.0);         // Turns to beacon at -85 degrees
        gyroHold( TURN_SPEED,   -70.0, 0.5);    // Hold  0 Deg heading for a 1/2 second
        shootP (1.8);
        gyroDrive(DRIVE_SPEED,   20.0, -70.0);    // Drive forward at 0 degrees
        followLine(2.0);
        beaconPush(0.2);    //Read Beacon Color and push correct button
        gyroDrive(DRIVE_SPEED, -10.0, -70.0);    // Drive backwards at -85 degrees
        beaconInit(0.2);
        gyroTurn( TURN_SPEED,   20.0);         // Turn back to 0 degrees
        gyroHold( TURN_SPEED,   20.0, 0.5);    // Holds turn
        gyroDrive(DRIVE_SPEED,  45.0, 20.0);    // Drive backwards to second beacon
        gyroTurn( TURN_SPEED,   -70.0);         // Turns to beacon at -85 degrees
        gyroHold( TURN_SPEED,   -70.0, 0.5);    // Hold  0 Deg heading for a 1/2 second
        followLine(2.0);
        ...
        beaconPush(0.2);    //Read Beacon Color and push correct button
        beaconInit(0.2);
        gyroDrive(1.0, -52.0, -70.0);    // Drive backwards -85 degrees
        ...
        while (opModeIsActive() && (holdTimer.time() < holdTime)) {
            ...
            if (touchSensor.isPressed()) {
                ...
                if (WHITE_THRESHOLD - lightSensor.getRawLightDetected() <= 0) {
                    robot.leftMotor.setPower((.15d) - (WHITE_THRESHOLD - lightSensor.getLightDetected()));
                    robot.rightMotor.setPower(.15d);

                } else {
                    robot.leftMotor.setPower(.15d);
                    robot.rightMotor.setPower((.15d) + (WHITE_THRESHOLD - lightSensor.getLightDetected()));
                }
            }
        }
