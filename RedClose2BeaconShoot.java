...
/**
 * Created by JAEGER on 11/26/2016.
 */

@Autonomous(name="Compbot: RedCloseFarBNearShoot", group="RED")
@Disabled
public class RedClose2BeaconShoot extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        gyroDrive(0.4,   10.0, 0.0);    // Drive forward at 0 degrees
        gyroTurn( 0.6,   25.0);         // Turns towards beacon
        gyroHold( TURN_SPEED,   23.0, 0.25);    // Holds turn
        gyroDrive(DRIVE_SPEED,  44.0, 23.0);    // Drive forward at -20 degrees (94 inches to reach white line without ODS)
        //encoderDrive(0.5, -19.0, 19.0, 3.0);    //Used to turn robot to beacon to begin following line
        gyroTurn( TURN_SPEED,   90.0);         // Turns to beacon at -85 degrees
        gyroHold( TURN_SPEED,   90.0, .5);    // Hold  0 Deg heading for a 1/2 second
        shootP (3.0);
        gyroDrive(DRIVE_SPEED,   23.0, 90.0);    // Drive forward at 0 degrees
        followLine(3.0);
        beaconPush(0.5);    //Read Beacon Color and push correct button
        gyroDrive(DRIVE_SPEED, -10.0, 90.0);    // Drive backwards at -85 degrees
        beaconInit(0.3);
        gyroTurn( TURN_SPEED,   0.0);         // Turn back to 0 degrees
        gyroHold( TURN_SPEED,   0.0, .75);    // Holds turn
        gyroDrive(DRIVE_SPEED,  49.0, 0.0);    // Drive backwards to second beacon
        gyroTurn( TURN_SPEED,   90.0);         // Turns to beacon at -85 degrees
        ...
        beaconPush(0.5);    //Read Beacon Color and push correct button
        beaconInit(0.3);
        gyroDrive(1.0, -60.0, 90.0);    // Drive backwards -85 degrees
        ...
        while (opModeIsActive() && (holdTimer.time() < holdTime)) {
            ...
            if (touchSensor.isPressed()) {
                ...
                if (WHITE_THRESHOLD - lightSensor.getRawLightDetected() <= 0) {
                    robot.leftMotor.setPower((.14d)+ (WHITE_THRESHOLD - lightSensor.getLightDetected()));
